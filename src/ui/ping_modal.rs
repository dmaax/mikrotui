    ...
    match &app.ping_state {
        ...
        PingState::InputtingTarget { input } => {
            ...
            let text = vec![
                ...
                Line::from(Span::styled(
                    " (Press Enter to start Ping | Esc to cancel) ",
                    t.muted_text,
                )),
            ];
        ...
        PingState::Running { target } => {
            ...
            let text = vec![
                ...
                Line::from(""),
                Line::from(Span::styled(
                    " Please wait for response from MikroTik... ",
                    t.warning,
                )),
            ];
        ...
        PingState::Completed { result } => {
            ...
            let block_main = Block::default()
                ...
                .border_style(t.border_focus)
                .title(Span::styled(
                    format!(" 📡 Ping Results: {} ", result.target),
                    t.title,
                ));
            ...
            // Summary Stats Bar
            let loss_style = if result.packet_loss_pct == 0 {
                t.success
            } else {
                t.danger
            };
            let stats_spans = vec![
                ...
                Span::styled(
                    format!("{}%", result.packet_loss_pct),
                    loss_style.add_modifier(Modifier::BOLD),
                ),
                Span::raw(" | "),
            ...
            let stats_p = Paragraph::new(Line::from(stats_spans)).block(
                Block::default()
            ...
            let rows = result.sequences.iter().map(|s| {
                let status_style = if s.status == "ok" || s.rtt_ms > 0 {
                    t.success
                } else {
                    t.danger
                };
                Row::new(vec![
                    ...
                    Cell::from(format!("{} ms", s.rtt_ms))
                        .style(t.warning.add_modifier(Modifier::BOLD)),
